    ...
    void updateworld()        // main game update loop
    {
        ...
        if(connected)
        {
            if(player1->state == CS_DEAD)
            {
                ...
                static int lastrespawn = 0;
                if(lastmillis - lastrespawn > 1000)  // wait 1 second before respawning
                {
                    lastrespawn = lastmillis;
                    game::respawnself();             // autorespawn
                }
            }
            else if(!intermission)
            {
                if(player1->ragdoll) cleanragdoll(player1);
                if(rl_bridge::connected())
                {
                    player1->ammo[GUN_RIFLE] = 200;
                    player1->gunselect = GUN_RIFLE;
                }
                rl_bridge::init();
                if(rl_bridge::connected())
                {
                    char jsonbuf[1024];
                    // build state from player1
                    fpsent *enemy = NULL;
                    float enemy_dist = 999.f, enemy_angle = 0.f;
                    int enemy_visible = 0;
                    loopv(players)
                    {
                        fpsent *e = players[i];
                        if(e == player1 || e->state != CS_ALIVE) continue;
                        float d = sqrtf(e->o.squaredist(player1->o));
                        if(d < enemy_dist) { enemy = e; enemy_dist = d; }
                    }
                    if(enemy)
                    {
                        enemy_angle = -atan2(enemy->o.x-player1->o.x, enemy->o.y-player1->o.y)/RAD - player1->yaw;
                        while(enemy_angle < -180.f) enemy_angle += 360.f;
                        while(enemy_angle >  180.f) enemy_angle -= 360.f;
                        vec targ;
                        enemy_visible = ai::getsight(player1->o, player1->yaw, player1->pitch, enemy->o, targ,
                            100.f, 90.f, 90.f) ? 1 : 0;
                    }
                    ...
                    {
                        fpsent *e = players[i];
                        if(e != player1 && e->state == CS_ALIVE) num_enemies++;
                    }
                    float ray_dists[8];
                    float ray_range = 200.f;
                    for(int ri = 0; ri < 8; ri++)
                    {
                        float ray_yaw = player1->yaw + ri * 45.f;
                        vec dir;
                        vecfromyawpitch(ray_yaw, 0, 1, 0, dir);
                        vec hitpos;
                        float hit = raycubepos(player1->o, dir, hitpos, ray_range, RAY_CLIPMAT|RAY_SKIPFIRST);
                        ray_dists[ri] = (hit < 0 ? ray_range : hit) / ray_range;
                    }
                    snprintf(jsonbuf, sizeof(jsonbuf),
                    "{\"pos_x\":%.2f,\"pos_y\":%.2f,\"pos_z\":%.2f,"
                    "\"yaw\":%.2f,\"pitch\":%.2f,"
                    "\"health\":%d,\"ammo\":%d,"
                    "\"enemy_visible\":%d,\"enemy_dist\":%.2f,\"enemy_angle_diff\":%.2f,"
                    "\"frags\":%d,\"deaths\":%d,"
                    "\"vel_x\":%.2f,\"vel_y\":%.2f,\"vel_z\":%.2f,"
                    "\"onground\":%d,\"num_enemies\":%d,"
                    "\"enemy_health\":%d,\"blocked\":%d,"
                    "\"ray0\":%.3f,\"ray1\":%.3f,\"ray2\":%.3f,\"ray3\":%.3f,"
                    "\"ray4\":%.3f,\"ray5\":%.3f,\"ray6\":%.3f,\"ray7\":%.3f,"
                    "\"dead\":0}\n",
                    player1->o.x, player1->o.y, player1->o.z,
                    player1->yaw, player1->pitch,
                    player1->health, player1->ammo[player1->gunselect],
                    enemy_visible, enemy_dist, enemy_angle,
                    player1->frags, player1->deaths,
                    player1->vel.x, player1->vel.y, player1->vel.z,
                    (player1->physstate >= PHYS_SLOPE) ? 1 : 0,
                    num_enemies,
                    enemy ? enemy->health : 100,
                    player1->blocked ? 1 : 0,
                    ray_dists[0], ray_dists[1], ray_dists[2], ray_dists[3],
                    ray_dists[4], ray_dists[5], ray_dists[6], ray_dists[7]
                );
                    rl_bridge::RLAction act = rl_bridge::exchange(jsonbuf);
                    if(act.valid && !act.reset)
                    {
                        player1->move   = (act.move_fb >  0.3f) ? 1 : (act.move_fb < -0.3f) ? -1 : 0;
